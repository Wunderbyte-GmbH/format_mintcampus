...
class toolbox {
     ...
     * @param stdClass $coursesectionimage Section information from its row in the 'format_mintcampus_image' table.
     ...
     * @param array $sectionimage Section information from its row in the 'format_mintcampus_image' table.
    ...
    public function setup_displayed_image($sectionimage, $sectionfile, $courseid, $sectionid, $format) {
        ...
        if (!empty($sectionfile)) {
            ...
            $tmproot = make_temp_directory('mintcampusformatdisplayedimagecontainer');
            ...
            if (!empty($data)) {
                ...
                $existingfiles = $fs->get_area_files($coursecontext->id, 'format_mintcampus', 'displayedsectionimage', $sectionid);
    ...
    public static function delete_images($courseid) {
        ...
        $coursecontext = \context_course::instance($courseid);
        // Images.
        $images = $fs->get_area_files($coursecontext->id, 'format_mintcampus', 'sectionimage');
        foreach ($images as $image) {
            $image->delete();
        }

        // Displayed images.
        $displayedimages = $fs->get_area_files($coursecontext->id, 'format_mintcampus', 'sectionimage');
    ...
    public static function delete_image($sectionid, $courseid) {
        ...
        if (!empty($coursesectionimage)) {
            ...
            if ($lock) {
                ...
                // Remove existing displayed image.
                $displayedfiles = $fs->get_area_files($coursecontext->id, 'format_mintcampus', 'displayedsectionimage', $sectionid);
