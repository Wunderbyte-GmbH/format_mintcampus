...
class restore_format_mintcampus_plugin extends restore_format_plugin {
    ...
    protected function define_course_plugin_structure() {
        ...
        /*
         * This is defines the nested tag within 'plugin_format_mintcampus_course' to allow '/course/plugin_format_mintcampus_course' in
    ...
    /**
     * Process the 'plugin_format_mintcampus_course' element within the 'course' element in the 'course.xml' file in the '/course'
     * folder of the zipped backup 'mbz' file.
     */
    public function process_mintcampus($data) {
        global $DB;
    ...
    public function after_restore_course() {
        ...
        // Sort out the files if old backup.  mintcampus image records already created with the section restore.
    ...
    protected function define_section_plugin_structure() {
        ...
        $paths[] = new restore_path_element('mintcampussection', $elepath);

        return $paths; // And we return the interesting paths.
    }

    /**
     * Process the 'plugin_format_mintcampus_section' element within the 'section' element in the 'section.xml' file in the
     * '/sections/section_sectionid' folder of the zipped backup 'mbz' file.
     */
    public function process_mintcampussection($data) {
        global $DB;

        $data = (object) $data;

        /* Allow this to process even if not in the mintcampus format so that our event observer on 'course_restored'
           can perform a clean up of restored mintcampus image files after all the data is in place in the database
        ...
        if ($this->step->get_task()->get_target() == backup::TARGET_NEW_COURSE) {
            ...
            if (empty($data->contenthash)) {
                ...
                if (!empty($data->image)) {
                    ...
                    // Contenthash later!
                    $newid = $DB->insert_record('format_mintcampus_image', $newimagecontainer, true);
                }
            } else {
                $oldsectionid = $data->sectionid;
                $this->set_mapping('mintcampusimage', $oldsectionid, $newsectionid, true);
                $this->add_related_files('format_mintcampus', 'sectionimage', 'mintcampusimage');
