...
class MoodleQuickForm_sectionfilemanager extends MoodleQuickForm_filemanager implements templatable {
        ...
        if (!$failure) {
            ...
            if ($lock) {
                ...
                if ($outdata->sectionimage == '1') {
                    ...
                    $files = $fs->get_area_files($coursecontext->id, 'format_mintcampus', 'sectionimage', $sectionid);
                        ...
                        if (!$file->is_directory()) {
                            ...
                            if ($sectionimage) {
                                if (($contenthash !== $sectionimage->contenthash) || ($filename !== $sectionimage->image)) {
                                    ...
                                    try {
                                        $DB->update_record('format_mintcampus_image', $sectionimage);
                                    } catch (\Exception $e) {
                                        if (!defined('BEHAT_SITE_RUNNING')) {
                                            $lock->release();
                                        }
                                ...
                                $sectionimage->displayedimagestate = 0; // Not generated.
                                try {
                                    $sectionimage->id = $DB->insert_record('format_mintcampus_image', $sectionimage, true);
                                } catch (\Exception $e) {
                                    if (!defined('BEHAT_SITE_RUNNING')) {
                                        $lock->release();
                                    }
    ...
    private function delete_existing_image($coursecontextid, $courseid, $sectionid, $fs, $lock) {
        ...
        $existingfiles = $fs->get_area_files($coursecontextid, 'format_mintcampus', 'displayedsectionimage', $sectionid);
